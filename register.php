	...
	if (empty($_POST["name"])) {
		...
		if (!preg_match("/^[a-zA-Z ]*$/", $name)){
			...
			if (empty($_POST["uname"])) {
				$usernameErr = "Por favor insira seu nome de usuário";
				$uname = "";
			}else{
				$uname = $_POST['uname'];

				if (empty($_POST["upassword"])) {
					$passwordErr = "Por favor, insira sua senha";
					$upassword = "";
				}else{
					$upassword = $_POST['upassword'];

					if (empty($_POST["ic"])){
						$icErr = "Por favor insira o seu número IC";
					}else{
						if(!preg_match("/^[0-9 -]*$/", $ic)){
							...
							if (empty($_POST["email"])){
								$emailErr = "Por favor, indique o seu endereço de e-mail";
							}else{
								if (filter_var($email, FILTER_VALIDATE_EMAIL)){
									...
									if (empty($_POST["contact"])){
										$contactErr = "Por favor, digite seu número de telefone";
									}else{
										if(!preg_match("/^[0-9 -]*$/", $contact)){
											$contactErr = "Por favor insira um número de telefone válido";
											$contact = "";
										}else{
											...
											if (empty($_POST["gender"])){
												...
												if (empty($_POST["address"])){
													$addressErr = "Por favor insira seu endereço";
													$address = "";
												}else{
